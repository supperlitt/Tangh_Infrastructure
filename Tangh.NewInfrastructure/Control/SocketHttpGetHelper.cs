...
namespace Tangh.NewInfrastructure
{
    public class SocketHttpGetHelper
    {
        ...
        public SocketHttpGetHelper(string ip, int port)
        {
            this.ip = ip;
            this.port = port;
        }
...
{1}";
                    ...
                    if (GetHandler != null && !string.IsNullOrEmpty(action))
                    {
                        try
                        {
                            ...
                            int length = Encoding.GetEncoding("gbk").GetBytes(result).Length;
                            string data = string.Format(headStr, length, result);
                            client.Send(Encoding.GetEncoding("gbk").GetBytes(data), SocketFlags.None);
                        }
                        catch { }
                        finally
                        {
