...
namespace Tangh.NewInfrastructure.Email
{
    ...
    public class EMailHttpHelper
    {
        ...
        public EMailHttpHelper()
        {
            Random rand = new Random(DateTime.Now.Millisecond);
            this.index = rand.Next(0, 3);
            this.cc = new CookieContainer();
        }

        public EMailHttpHelper(CookieContainer cc)
        {
            Random rand = new Random(DateTime.Now.Millisecond);
            this.index = rand.Next(0, 3);
            this.cc = cc;
        ...
        public Stream PostAndGetStream(string targetURL, string formData, string contentType, string referer, bool allowAutoRedirect)
        {
            ...
            try
            {
                CurrentUrl = response.ResponseUri.AbsoluteUri;
            }
            catch
            {
            }
        ...
        ///</summary>
        public string GetAndGetHtml(string targetURL, string contentType, string referer, bool allowAutoRedirect, Encoding encoding, bool isgzip = false)
        {
            Stream stream = GetAndGetStream(targetURL, contentType, referer, allowAutoRedirect);
            if (isgzip)
            {
                GZipStream gs = new GZipStream(stream, CompressionMode.Decompress);
                string html = new StreamReader(gs, encoding).ReadToEnd();
                return html;
            }
            else
            {
        ...
        {
            Stream stream = GetAndGetStream(url, null, null, false);
            if (stream == null)
            {
