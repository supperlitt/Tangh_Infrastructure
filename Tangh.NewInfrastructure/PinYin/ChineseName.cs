...
namespace Tangh.NewInfrastructure.PinYin
{
    public class ChineseName
    {
        ...
        {
            string str = ""; int count = rnd.Next(1, 3); for (int i = 0; i < count; i++)
            {
                while (true)
                {
                    int ACode = 1601 + rnd.Next(999); string strtemp = Encoding.Default.GetString(new byte[] { (byte)(ACode / 100 + 160), (byte)(ACode % 100 + 160) });
                    if (strtemp != "?") { str += strtemp; break; }
                }
            }
